...
RegisterNetEvent('RL-jobgaragemenu:client:vehCategories', function(coords)
    local categoryMenu = {
        {
    ...
    for category, label in pairs(Config.Categories) do
        ...
        if found then            
            categoryMenu[#categoryMenu + 1] = {
                header = label,
                params = {
                    event = 'RL-jobgaragemenu:client:openVehCats',
                    args = {
                        catName = category,
                        coords  = spawnpoint,
                        jobname = jobname,
    ...
    categoryMenu[#categoryMenu + 1] = {
        ...
        params = {
            event = 'RL-jobgaragemenu:client:homeMenu'
        }
    }
    exports['qb-menu']:openMenu(categoryMenu)
end)

RegisterNetEvent('RL-jobgaragemenu:client:openVehCats', function(data)
    local vehicleMenu = {
        {
        ...
        },
        {
            ...
            params = {
                event = 'RL-jobgaragemenu:client:vehCategories'
            }
        }
    }
                ...
                if QBCore.Functions.GetPlayerData().job.name == Config.Vehicles[k].job then
                    if QBCore.Functions.GetPlayerData().job.grade.level == rank then
                        vehicleMenu[#vehicleMenu + 1] = {
                            ...
                            txt = data.catName,
                            params = {
                                event = 'RL-jobgaragemenu:client:takeOutVehicle',
                                args = {
                                    vehicle = v.model,
                                    coords = data.coords,
...
RegisterNetEvent('RL-jobgaragemenu:client:takeOutVehicle', function(data)
    TakeOutVehicle(data)
end)
    ...
    while true do
        if IsControlJustReleased(0, 38) and isInside then
            ...
            if IsPedInAnyVehicle(ped, false) then
                ...
                TriggerEvent('RL-jobgaragemenu:client:vehCategories', pos)
            end
        end
        Wait(10)
    end
